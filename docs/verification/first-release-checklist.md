...
- `ffmpeg` / `ffprobe` are installer-managed on Windows: the NSIS installer checks the install-root `dependencies\ffmpeg` tools after install or update, skips download when both tools already exist, and downloads plus verifies the pinned LGPL FFmpeg release asset only when either required tool is missing. The license profile is recorded in `docs/distribution/ffmpeg-license-profile.md`.
